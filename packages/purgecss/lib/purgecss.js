...
async function setOptions(configFile = CONFIG_FILENAME) {
    ...
    try {
        ...
        options = await new Promise(function (resolve) { resolve(_interopNamespace(require(t))); });
    }
    catch (err) {
...
class PurgeCSS {
    ...
    async extractSelectorsFromFiles(files, extractors) {
        ...
        for (const globfile of files) {
            let filesNames = [];
            try {
                await fs.promises.access(globfile, fs.constants.F_OK);
    ...
    async getPurgedCSS(cssOptions, selectors) {
        ...
        for (const option of processedOptions) {
            const cssContent = typeof option === "string"
                ? this.options.stdin
                    ? option
                    : await fs.promises.readFile(option, "utf-8")
                : option.raw;
